...
export class TrustNetApi {
    ...
    public static async generateBlockHeader(content: string, address: string, blockId: number, updateId: number, version: number = 1): Promise<IBlockHeader> {
    ...
    public static async signBlockHeader(header: IBlockHeader, privateKey: string): Promise<IBlockHeader> {
        const information = header.version + header.output_hash + header.address + header.block_id + header.update_id
		header.signature = await this.sign(information, privateKey)
        return header
	}

    public static async verifyBlockHeaderSignature(header: IBlockHeader, publicKey: string): Promise<boolean> {
        const information = header.version + header.output_hash + header.address + header.block_id + header.update_id
